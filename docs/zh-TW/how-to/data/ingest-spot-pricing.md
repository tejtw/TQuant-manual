...
!!! info
    本頁提供如何將股票價量資料 Ingest 到 TQuant Lab 的詳細指南，並說明相關的參數設定與步驟。
...
*   `os.environ['TEJAPI_KEY'] = "Your_Key"`
    *   用於識別使用者
*   `os.environ['ticker'] = "xxxx xxxx xxxx"`
    *   設置我們所需要的股票代碼
...
### 非價量資料 bundle（fundamentals bundle）需要的資訊：
...
!!! important
    請注意，`zipline ingest` 和 `zipline add` 指令在執行時會連接至 TEJ 伺服器以載入資料。這會 **消耗您的 TEJ API 流量**。
