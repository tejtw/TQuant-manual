...
!!! info
    本頁宏觀地介紹 TQuant Lab 的核心分析流程，從資料獲取、策略發想、回測執行到績效分析，為使用者建立一個整體的概念框架。
...
## 5. 常見陷阱與最佳實踐 (Pitfalls & Best Practices)

在量化研究的道路上，魔鬼往往藏在細節裡。請特別留意以下常見問題：

!!! failure "致命錯誤：向前看偏誤 (Look-ahead Bias)"
    這是最常見也最危險的錯誤。確保您的策略在 T 日做決策時，**絕對沒有** 使用到 T+1 日以後的資訊（例如：在週一開盤時就使用了週一的收盤價）。Zipline 的 Pipeline 機制已內建防呆，但自訂數據時仍需小心。

!!! warning "過度優化 (Overfitting)"
    不要為了讓回測曲線變漂亮而過度調整參數。一個參數組在過去 5 年表現完美，往往意味著它在未來會表現極差。請務必進行 **樣本外測試 (Out-of-Sample Testing)**。

!!! tip "不可忽視的成本：滑價與手續費"
    永遠不要假設交易是免費的。在 `initialize` 中務必設定 `set_commission` 和 `set_slippage`。
